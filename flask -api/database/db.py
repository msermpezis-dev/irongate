...
class Database:
    ...
    conn = pyodbc.connect("DRIVER={ODBC Driver 17 for SQL Server};"
                          ...
                          "DATABASE=ironbase;" 
                          "UID=sa;" 
                          "PWD=m3,J*?LPY\9?<%dz#~.XjaLA[4-FNfBN")
    c = conn.cursor()
    ...
    def data_entry_userdata(self, email, salt, iv, mcvalue, cvalue, emk):     #insert data into UserData table
        self.c.execute("INSERT INTO UserData (email, salt, iv, mcvalue, cvalue, emk) "
                       "VALUES(?, ?, ?, ?, ?, ?)",
                        (email, salt, iv, mcvalue, cvalue, emk,))
        self.conn.commit()

    def data_entry_entity(self, user_id, entity_name, entity_un, entity_pw, note=''):    #insert data into Entity table and returns id of inserted entity
        ent_id = self.c.execute("INSERT INTO Entity (entity_name, entity_un, entity_pw, note) "
                           "OUTPUT INSERTED.entity_id VALUES(?, ?, ?, ?)",
                       (entity_name, entity_un, entity_pw, note,)).fetchone()[0]
        self.conn.commit()
        self.data_entry_user_entity(user_id, ent_id)

    def data_entry_user_entity(self, user_id, entity_id):       #insert data into User_Entity relationship table
        self.c.execute("INSERT INTO User_Entity (user_id, entity_id) "
                       "VALUES(?, ?)",
                        (user_id, entity_id,))
        self.conn.commit()
    ...
    def get_entities_ids(self, user_id):     #returns False if user has any entities and returns them if he has
        self.c.execute("SELECT Entity.entity_id FROM Entity, User_Entity "
                       "WHERE User_Entity.entity_id = Entity.entity_id AND user_id = ? ", (user_id,))
        s = self.c.fetchall()
        return s
    ...
    def get_decryption_values_pwcheck(self, user_id):      #returns user values
        self.c.execute("SELECT salt, iv, cvalue FROM UserData WHERE user_id = ?", (user_id,))
        return self.c.fetchall()[0]

    def get_decryption_values_mpcheck(self, user_id):      #returns user values
        self.c.execute("SELECT salt, iv, mcvalue FROM UserData WHERE user_id = ?", (user_id,))
        return self.c.fetchall()[0]

    def get_decryption_values_only(self, user_id):      #returns user values
